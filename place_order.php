...
if ($productId > 0 && $quantity > 0 && !empty($shippingAddress) && !empty($contactNumber)) {
    $userId = $_SESSION['uid']; // Assuming you want to include the user ID

    // Prepare an SQL statement to insert the order into the orders table
    $stmt = $conn->prepare("INSERT INTO orders (user_id, product_id, quantity, shipping_address, contact_number) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("iiiss", $userId, $productId, $quantity, $shippingAddress, $contactNumber);
